...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="text" id="chn" name="chn" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO checklist(checklist_id, couple_id, checklist_name) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $chid, $cid, $chn);
...
// SQL query to fetch data from the checklist table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of checklist</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of checklist</h2></center>
    <table border="5">
        <tr>
            <th>checklist_id</th>
            <th>couple_id</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all checklist
        $sql = "SELECT * FROM checklist";
        $result = $connection->query($sql);

        // Check if there are any checklist
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $chid = $row['checklist_id']; // Fetch the checklist_id
                echo "<tr>
                    <td>" . $row['checklist_id'] . "</td>
                    <td>" . $row['couple_id'] . "</td>
                    <td>" . $row['checklist_name'] . "</td>
                    <td><a style='padding:4px' href='delete_checklist.php?BranchID=$chid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_checklist.php?BranchID=$chid'>Update</a></td> 
                </tr>";
            }
        } else {
