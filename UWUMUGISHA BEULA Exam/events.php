...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="text" id="enm" name="enm" required><br><br>

        <label for=edt>event_date:</label>
        <input type="date" id="edt" name="edt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO events(event_id, couple_id, event_name, event_date) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $eid, $cid, $enm, $edt);
     ...
     include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $eid = $row['event_id']; // Fetch the event_id
                echo "<tr>
                    <td>" . $row['event_id'] . "</td>
                    <td>" . $row['couple_id'] . "</td>
