...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="wdt">wedding_date:</label>
        <input type="date" id="wdt" name="wdt" required><br><br>

        <label for="wdl">wedding_location:</label>
        <input type="text" id="wdl" name="wdl" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO couples(couple_id, id, wedding_date, wedding_location) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $cid, $uid, $wdt, $wdl);
...
// SQL query to fetch data from the couples table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of couples</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of couples</h2></center>
    <table border="5">
        <tr>
            <th>couples_id</th>
            <th>user_id</th>
            <th>wedding_date</th>
            <th>wedding_location</th>
            <th>Delete</th>
            <th>Update</th>
        </tr>
        <?php
       include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all couples
        $sql = "SELECT * FROM couples";
        $result = $connection->query($sql);

        // Check if there are any couples
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $cid = $row['couple_id']; // Fetch the couple_id
                echo "<tr>
                    <td>" . $row['couple_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['wedding_date'] . "</td>
                    <td>" . $row['wedding_location'] . "</td>
                    <td><a style='padding:4px' href='delete_couples.php?couple_id=$cid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_couples.php?couple_id=$cid'>Update</a></td> 
                </tr>";
            }
        } else {
